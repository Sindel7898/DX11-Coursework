...
public:
	...
	void LightUI(int LightNumber); //UI CONFIG FOR LIGHTS
	void SetLightIntensity(int LightIntensitynum);
	void SetLightType(int LightType);
	void SetConeParams(float  innerCone, float outtercone);
	void SetLghtIntensity(int lightIntensity);
	void SetSpecularPower(int SpecularIntensity);
	...
	float getLightIntensity();			///< Get light orthographic matrix for shadow mapping, returns XMMATRIX
	int GetLightType();
	XMFLOAT2 GetConeParams();			///< Get light direction, returns float3
...
public:
	XMFLOAT3 Newposition{ 0.0f, -1.0f, 0.0f };
